...
export default function ContributePage() {
  ...
  const handleFile = (f: File|null) => {
    ...
    if (!allowed.includes(f.type)) { setFileErr("Only PDF, JPG, PNG, or DOCX files are accepted."); return; }
    if (f.size > 20 * 1024 * 1024) { setFileErr("File size must not exceed 20 MB."); return; }
    setFile(f);
  };

  const submit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!file) { setFileErr("Please select a file before submitting."); return; }
    setStatus("loading");
  ...
  const sel: React.CSSProperties = {
    ...
    outline:"none", cursor:"pointer", fontFamily:"inherit", transition:"border-color 0.17s",
  };
  ...
  if (status === "success") return (
    <div style={{ maxWidth:480, margin:"80px auto", textAlign:"center", animation:"fadeUp 0.5s ease" }}>
      ...
      <h2 style={{ fontSize:22, fontWeight:800, color:"#111", marginBottom:10 }}>Submission Received!</h2>
  ...
  return (
    <div style={{ maxWidth:640 }}>
      ...
      <div className="fade-up" style={{
        ...
        <p style={{ fontSize:14, opacity:0.9 }}>Help your fellow NTU students. Your submission will be reviewed by an admin before going live.</p>
      </div>
      ...
      <form onSubmit={submit} style={{ display:"flex", flexDirection:"column", gap:16 }}>
        ...
        <div className="section-card fade-up-1">
          <label style={{ display:"block", fontSize:11, fontWeight:700, color:"#aaa", letterSpacing:"0.08em", marginBottom:10, textTransform:"uppercase" }}>Your Roll Number</label>
          <input value={roll} onChange={e => setRoll(e.target.value)} placeholder="e.g. 25-NTU-CS-FL-1124" required className="input-field" />
          {dept && (
          ...
          {roll.length > 8 && !dept && <p style={{ fontSize:12, color:"#dc2626", marginTop:8 }}>Invalid format. Correct example: 25-NTU-CS-FL-1124</p>}
        </div>
          ...
          <div style={{ display:"flex", flexDirection:"column", gap:14 }}>
            <div>
              ...
              <select value={teacherId} onChange={e => setTeacherId(e.target.value)} required disabled={!dept} style={{ ...sel, opacity: dept ? 1 : 0.5 }}>
                <option value="">{!dept ? "Enter your roll number first" : teachers.length === 0 ? "No teachers found" : "Select a teacher"}</option>
            ...
            <div>
              ...
              <select value={subjectId} onChange={e => setSubjectId(e.target.value)} required disabled={!teacherId} style={{ ...sel, opacity: teacherId ? 1 : 0.5 }}>
                <option value="">{!teacherId ? "Select a teacher first" : "Select a subject"}</option>
            ...
            <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:12 }}>
              {[
                ...
                { label:"Semester",   val:semester,    set:setSemester,  opts:["1","2","3","4","5","6","7","8"].map(s => [s,`Semester ${s}`]) },
        ...
        <div className="section-card fade-up-3">
          <label style={{ display:"block", fontSize:11, fontWeight:700, color:"#aaa", letterSpacing:"0.08em", marginBottom:12, textTransform:"uppercase" }}>Upload File</label>
          <div
          ...
          >
            ...
            {file ? (
              <>
                ...
                <p style={{ fontSize:12, color:"#777" }}>{(file.size/1024/1024).toFixed(1)} MB &middot; Click or drag to replace</p>
              </>
            ) : (
              <>
                <p style={{ fontWeight:600, color:"#444", fontSize:14, marginBottom:4 }}>Click to browse or drag & drop your file here</p>
                <p style={{ fontSize:12, color:"#aaa" }}>Accepted: PDF, JPG, PNG, DOCX &middot; Max 20 MB</p>
              </>
            )}
        ...
        {status === "error" && msg && (
          <div style={{ background:"#fef2f2", border:"1px solid #fecaca", borderRadius:10, padding:"12px 16px", color:"#dc2626", fontSize:13, fontWeight:500 }}>{msg}</div>
        )}
