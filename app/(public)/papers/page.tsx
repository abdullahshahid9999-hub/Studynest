...
export default function PapersPage() {
  ...
  const handleTeacher = async (tid: string, tname: string) => {
    setTeacherId(tid); setTeacherName(tname); setSubjectId(""); setPapers([]); setSearched(false);
  ...
  return (
    <div style={{ maxWidth: 940 }}>
      ...
      <div className="fade-up" style={{
        ...
        <p style={{ fontSize: 15, opacity: 0.85, maxWidth: 480 }}>Enter your roll number to instantly find past papers for your department, teacher, and subject.</p>
      </div>
      ...
      <div className="section-card fade-up-1">
        ...
        <div style={{ display: "flex", gap: 10 }}>
          <input value={roll} onChange={e => setRoll(e.target.value)}
            ...
            placeholder="25-NTU-CS-FL-1124" className="input-field" />
          <button onClick={handleRoll} disabled={!parseRoll(roll)} className="btn-primary" style={{ padding: "11px 22px", flexShrink: 0, opacity: parseRoll(roll) ? 1 : 0.45 }}>
            Search
          </button>
        </div>
        {dept && (
        ...
        {roll.length > 8 && !parseRoll(roll) && (
          <p style={{ fontSize: 12, color: "#dc2626", marginTop: 8 }}>Format galat hai. Sahi example: 25-NTU-CS-FL-1124</p>
        )}
      </div>
      ...
      {step >= 2 && (
        <div className="section-card fade-up-2">
          ...
          {teachers.length === 0 ? (
            <p style={{ fontSize: 13, color: "#bbb" }}>Koi teacher nahi mila is department mein.</p>
          ) : (
      ...
      {step >= 3 && (
        <div className="section-card fade-up-2">
          ...
          {subjects.length === 0 ? (
            <p style={{ fontSize: 13, color: "#bbb" }}>Is teacher ka koi subject nahi mila.</p>
          ) : (
            <div style={{ display: "flex", flexWrap: "wrap", gap: 8 }}>
              {subjects.map(s => (
      ...
      {searched && (
        loading ? (
          <div className="section-card" style={{ textAlign: "center", padding: "48px 20px" }}>
            ...
            <p style={{ fontSize: 14, color: "#bbb", marginTop: 12 }}>Papers dhoondh rahe hain…</p>
          </div>
        ) : papers.length === 0 ? (
          <div className="section-card" style={{ textAlign: "center", padding: "56px 20px" }}>
            <div style={{ width: 60, height: 60, borderRadius: "50%", background: "#f5f5f5", display: "flex", alignItems: "center", justifyContent: "center", margin: "0 auto 16px", fontSize: 26 }}>
              📭
            </div>
            <p style={{ fontWeight: 700, color: "#333", fontSize: 15, marginBottom: 8 }}>Koi paper nahi mila</p>
            <p style={{ fontSize: 13, color: "#aaa", marginBottom: 20 }}>{subjectName} ke liye abhi koi paper available nahi. Pehle contribute karo!</p>
          ...
          <div className="fade-up-3">
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 14 }}>
              <p style={{ fontSize: 13, color: "#777" }}>
                <b style={{ color: "#111", fontSize: 15 }}>{papers.length}</b> paper{papers.length !== 1 ? "s" : ""} for <b style={{ color: "#111" }}>{subjectName}</b>
              </p>
            </div>
              ...
              {papers.map((p: any, idx: number) => (
                <div key={p.id} className="card-hover" style={{
                  ...
                  <div style={{ display: "flex", gap: 8, paddingTop: 14, borderTop: "1px solid #f0f0f0" }}>
                    <button onClick={() => handleView(p)} disabled={busy === p.id} className="btn-primary" style={{ flex: 1, padding: "9px", opacity: busy === p.id ? 0.6 : 1 }}>
        ...
        <div style={{ position: "fixed", inset: 0, zIndex: 9999, background: "#0f0f0f", display: "flex", flexDirection: "column", animation: "fadeIn 0.2s ease" }}>
          <div style={{ background: "#fff", padding: "10px 18px", display: "flex", justifyContent: "space-between", alignItems: "center", flexShrink: 0, borderBottom: "1px solid #e8e8e8", gap: 12 }}>
            <div style={{ minWidth: 0, flex: 1 }}>
              ...
              <div style={{ fontSize: 12, color: "#888" }}>{viewer.paper.subjects?.course_code} &middot; {viewer.paper.exam_type} &middot; Sem {viewer.paper.semester} &middot; {viewer.paper.term} {viewer.paper.year}</div>
            </div>
