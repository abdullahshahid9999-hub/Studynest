...
export default function LeaderboardPage() {
  ...
  const medal = (rank: number) => {
    if (rank === 1) return { bg:"linear-gradient(135deg,#f6d365,#fda085)", label:"1st" };
    if (rank === 2) return { bg:"linear-gradient(135deg,#e0e0e0,#bdbdbd)", label:"2nd" };
    if (rank === 3) return { bg:"linear-gradient(135deg,#f093fb,#c471ed)", label:"3rd" };
    return { bg:"#f5f5f5", label:`#${rank}` };
  };
    ...
    <div style={{ maxWidth:820 }}>
      ...
      <div className="fade-up" style={{
        ...
        <p style={{ fontSize:15, opacity:0.9 }}>
          {loading ? "Loading contributors…" : `${entries.length} student${entries.length !== 1 ? "s" : ""} have contributed papers`}
        </p>
      </div>
          ...
          {top3.map((e, i) => {
            ...
            return (
              <div key={e.id} className="card-hover" style={{
                background:"#fff", borderRadius:16, border:"1px solid #e8e8e8", overflow:"hidden",
                animation:`fadeUp 0.4s ${i*0.08}s ease both`, cursor:"pointer",
              }} onClick={() => openProfile(e.roll_number)}>
                <div style={{ background:m.bg, padding:"20px 20px 24px" }}>
                ...
                <div style={{ padding:"14px 20px" }}>
                  ...
                  {e.department_code && (
                    <span style={{ background:"#eef2ff", color:"#3b5bdb", padding:"2px 8px", borderRadius:5, fontSize:11, fontWeight:700 }}>{e.department_code}</span>
                  )}
                </div>
              </div>
            );
      ...
      <div className="fade-up-2" style={{ background:"#fff", border:"1px solid #e8e8e8", borderRadius:16, overflow:"hidden" }}>
        <div style={{ padding:"16px 20px", borderBottom:"1px solid #f0f0f0", display:"flex", justifyContent:"space-between", alignItems:"center", gap:12 }}>
          <span style={{ fontSize:14, fontWeight:700, color:"#111" }}>All Contributors</span>
          <input value={search} onChange={e => setSearch(e.target.value)}
            placeholder="Search by roll number…"
            style={{ padding:"8px 13px", border:"1.5px solid #e0e0e0", borderRadius:8, fontSize:13, outline:"none", width:220, fontFamily:"inherit", transition:"border-color 0.17s" }}
        ...
        {loading ? (
          <div style={{ padding:56, textAlign:"center" }}>
            ...
            <p style={{ fontSize:14, color:"#bbb", marginTop:12 }}>Loading leaderboard…</p>
          </div>
        ) : filtered.length === 0 ? (
          <div style={{ padding:"56px 20px", textAlign:"center" }}>
            <div style={{ fontSize:36, marginBottom:12 }}>🏆</div>
            <p style={{ fontWeight:700, color:"#333", marginBottom:6 }}>{search ? "No results found." : "No contributors yet."}</p>
            {!search && <a href="/contribute" style={{ display:"inline-block", marginTop:16, padding:"9px 20px", background:"linear-gradient(135deg,#43e97b,#38f9d7)", color:"#fff", borderRadius:9, fontSize:13, fontWeight:700, textDecoration:"none" }}>Be the First to Contribute</a>}
          </div>
        ) : (
          <table style={{ width:"100%", borderCollapse:"collapse" }}>
            <thead>
              <tr style={{ background:"#fafafa", borderBottom:"1px solid #f0f0f0" }}>
                {["Rank","Roll Number","Department","Approved",""].map(h => (
                ...
                return (
                  <tr key={e.id} className="table-row" style={{ borderBottom:"1px solid #f7f7f7", animation:`fadeUp 0.35s ${idx*0.04}s ease both` }}>
                    <td style={{ padding:"12px 16px" }}>
                      <span style={{ display:"inline-block", padding:"3px 10px", borderRadius:7, fontSize:12, fontWeight:800, background: rank <= 3 ? m.bg : "#f5f5f5", color: rank <= 3 ? "#fff" : "#aaa" }}>
                        {m.label}
                      </span>
                    </td>
                    <td style={{ padding:"12px 16px", fontFamily:"monospace", fontWeight:700, fontSize:13, color:"#111" }}>{e.roll_number}</td>
                    <td style={{ padding:"12px 16px" }}>
                      {e.department_code && <span style={{ background:"#eef2ff", color:"#3b5bdb", padding:"3px 9px", borderRadius:6, fontSize:11, fontWeight:700 }}>{e.department_code}</span>}
                    </td>
                    <td style={{ padding:"12px 16px" }}>
                    ...
                    <td style={{ padding:"12px 16px" }}>
                      <button onClick={() => openProfile(e.roll_number)} style={{
                        padding:"5px 14px", background:"transparent", border:"1.5px solid #e0e0e0",
                        borderRadius:7, cursor:"pointer", fontSize:12, color:"#555", fontWeight:600, fontFamily:"inherit", transition:"all 0.15s",
                      }}
      ...
      {profile && (
        <div className="modal-overlay" onClick={() => setProfile(null)}>
          <div className="modal-box" onClick={e => e.stopPropagation()}>
            {profile.loading ? (
              <div style={{ padding:40, textAlign:"center", color:"#bbb" }}>Loading profile…</div>
            ) : (
              <>
                <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start", marginBottom:22 }}>
                  ...
                  <button onClick={() => setProfile(null)} style={{ background:"#f5f5f5", border:"none", borderRadius:8, width:34, height:34, cursor:"pointer", fontSize:17, color:"#888" }}>✕</button>
                </div>
                <div style={{ display:"grid", gridTemplateColumns:"repeat(3,1fr)", gap:10, marginBottom:24 }}>
                  {[
                    { l:"Approved", v:profile.c?.total_approved, grad:"linear-gradient(135deg,#43e97b,#38f9d7)" },
                    { l:"Pending",  v:profile.c?.total_pending,  grad:"linear-gradient(135deg,#f6d365,#fda085)" },
                    { l:"Rejected", v:profile.c?.total_rejected, grad:"linear-gradient(135deg,#f093fb,#f5576c)" },
                  ].map(({ l, v, grad }) => (
                ...
                <div style={{ fontSize:11, fontWeight:700, color:"#bbb", textTransform:"uppercase", letterSpacing:"0.07em", marginBottom:10 }}>Submission History</div>
                  ...
                  <table style={{ width:"100%", borderCollapse:"collapse", fontSize:13 }}>
                    ...
                    <tbody>
                      {profile.papers.map((p: any, i: number) => (
                        <tr key={i} className="table-row" style={{ borderTop:"1px solid #f5f5f5" }}>
                          <td style={{ padding:"10px 12px" }}>
                            <div style={{ fontWeight:600, color:"#111" }}>{p.subjects?.name}</div>
                      ...
                      {!profile.papers.length && (
                        <tr><td colSpan={4} style={{ padding:28, textAlign:"center", color:"#bbb", fontSize:13 }}>No submissions yet.</td></tr>
                      )}
                    </tbody>
                  </table>
                </div>
